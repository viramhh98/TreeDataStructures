...
class RightSkewedTree {
    public static void main(String[] args) {
        // insert the values in binary search tree and then it will convert to right skewed tree

        treeObj1 bst = new treeObj1();
        ...
        bst.printLevel();
        bst.visualizeTree(bst.headRightSkewedTree, 0);
    }
}

class treeObj1 {
    Node head;
    Node headRightSkewedTree;
    ...
    void inorder(Node n) {
        ...
        inorder(n.leftChild);
        InsertLeftSkewedTree(n.data);
        inorder(n.rightChild);
    }

    void InsertLeftSkewedTree(int data) {
        if (headRightSkewedTree == null) {
            Node n = new Node(data);
            headRightSkewedTree = n;
        } else {
            Node traverse = headRightSkewedTree;
            Node n = new Node(data);
            while (traverse.rightChild != null) {
                traverse = traverse.rightChild;
            }
            traverse.rightChild = n;
        }
